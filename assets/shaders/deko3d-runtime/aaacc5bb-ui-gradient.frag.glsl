...
layout(location = 1) flat in vec2 _vs2fs_location1;
...
layout(location = 5) smooth in vec2 _vs2fs_location5;
layout(location = 6) flat in vec2 _vs2fs_location6;
layout(location = 7) flat in vec2 _vs2fs_location7;
layout(location = 8) flat in vec4 _vs2fs_location8;
layout(location = 9) flat in float _vs2fs_location9;
layout(location = 10) flat in float _vs2fs_location10;
layout(location = 11) flat in vec4 _vs2fs_location11;
layout(location = 12) flat in float _vs2fs_location12;
...
void main() {
    GradientVertexOutput in_ = GradientVertexOutput(_vs2fs_location0, _vs2fs_location1, _vs2fs_location2, _vs2fs_location3, _vs2fs_location4, _vs2fs_location5, _vs2fs_location6, _vs2fs_location7, _vs2fs_location8, _vs2fs_location9, _vs2fs_location10, _vs2fs_location11, _vs2fs_location12, gl_FragCoord);
    float g_distance = 0.0;
    bool _e4 = enabled(in_.flags, RADIAL);
    if (_e4) {
        float _e9 = radial_distance(in_.point, in_.g_start, in_.dir.x);
        g_distance = _e9;
    } else {
        bool _e12 = enabled(in_.flags, CONIC);
        if (_e12) {
            float _e17 = conic_distance(in_.dir.x, in_.point, in_.g_start);
            g_distance = _e17;
        } else {
            float _e21 = linear_distance(in_.point, in_.g_start, in_.dir);
            g_distance = _e21;
        }
    }
    float _e22_ = g_distance;
    vec4 _e29 = interpolate_gradient(_e22_, in_.start_color, in_.start_len, in_.end_color, in_.end_len, in_.hint, in_.flags);
    bool _e32 = enabled(in_.flags, BORDER_ANY);
    if (_e32) {
        vec4 _e38 = draw_uinode_borderX_naga_oil_mod_XMJSXM6K7OVUTUOTVNFPW433EMUX(_e29, in_.point, in_.size, in_.radius, in_.border, in_.flags);
        _fs2p_location0 = _e38;
        return;
    } else {
        vec4 _e44 = draw_uinode_backgroundX_naga_oil_mod_XMJSXM6K7OVUTUOTVNFPW433EMUX(_e29, in_.point, in_.size, in_.radius, in_.border, in_.flags);
        _fs2p_location0 = _e44;
        return;
    }
}
