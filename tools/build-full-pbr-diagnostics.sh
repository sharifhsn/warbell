...
OUT_DIR=${PBR_DIAGNOSTIC_OUT_DIR:-"$ROOT_DIR/target/pbr-variants"}

[ -f "$SOURCE" ] || { echo "missing captured PBR WGSL: $SOURCE" >&2; exit 1; }
mkdir -p "$OUT_DIR"

if [ "$#" -eq 0 ]; then
  set -- "$ROOT_DIR"/tools/shader-variants/full-pbr/*.wgsl-body
else
  requested=
  for name in "$@"; do
    requested="$requested $ROOT_DIR/tools/shader-variants/full-pbr/$name.wgsl-body"
  done
  set -- $requested
fi

for body in "$@"; do
  [ -f "$body" ] || { echo "unknown PBR diagnostic: $body" >&2; exit 2; }
  name=$(basename "$body" .wgsl-body)
  wgsl="$OUT_DIR/$name.wgsl"
  rtk uv run "$ROOT_DIR/tools/replace-wgsl-entry-body.py" \
    --input "$SOURCE" --entry fragment --body "$body" --output "$wgsl"
...
echo "built full PBR diagnostics in $OUT_DIR"
